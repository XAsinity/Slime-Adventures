	...
	local function findSerializerHelperModule(moduleName)
		if script then
			local inst = script:FindFirstChild(moduleName)
		...
		if modules then
			local inst = modules:FindFirstChild(moduleName)
			if inst then return inst end
		end

		local direct = ServerScriptService:FindFirstChild(moduleName)
	...
	local WS_ATTR_MAP = {
		...
		ValuePerGrowth="vg", MutationCount="mc", MutationValueMult="mm",
		MutationLastType="mt", MutationLastApplied="ma", MutationHasPhysical="mp",
		MutationFoodStub="mf", MutationFoodStubCount="mfc", MutationSizeBoost="mbs",
		MutationHistory="mh", Tier="ti", FedFraction="ff",
	...
	local function ws_restore(player, list)
		...
		local function enforcePivot(model, desiredCF, label)
			if not model or typeof(desiredCF) ~= "CFrame" then return false end
			local prim = nil
			pcall(function() prim = model.PrimaryPart end)
			...
			if sidKey and restoredIds[sidKey] then
				...
				if existing then
					...
					if not skip_existing_remaining then
						...
						local spawnPositionOverride = nil
						local targetCF = nil
						if prim2 then
						...
						pcall(function()
							if existing then
								local ownerUidVal = (player and player.UserId) or e.ow
								if ownerUidVal == nil then
									local okOwner, existingOwner = pcall(function() return existing:GetAttribute("OwnerUserId") end)
									if okOwner then ownerUidVal = existingOwner end
								end
								mark_restored_instance(existing, tonumber(ownerUidVal) or ownerUidVal, nil)
							end
						end)
						pcall(function()
							if existing then
						...
						if pivotTarget then
							...
							if immediatePivot then
								...
								if not fallbackVec and prim2 then
									local okPos, primPos = pcall(function() return prim2.Position end)
							...
							if created then
								...
								pcall(function()
									if created then
										...
										if desiredPosition then
											spawnOverrideVector = desiredPosition
											created:SetAttribute("SpawnPosition", desiredPosition)
										end
									end
								end)
								local pivotTarget = targetCF
								...
								if pivotTarget then
									...
									if immediatePivot then
										...
										if not fallbackVec then
											local okPrim, prim = pcall(function() return created and created.PrimaryPart end)
											if okPrim and prim then
												local okPos, primPos = pcall(function() return prim.Position end)
												if okPos then fallbackVec = primPos end
											end
										end
	...
	local _foodDefinitionsCache = nil
	local function getFoodDefinitions()
		if _foodDefinitionsCache ~= nil then
			return _foodDefinitionsCache or nil
		end

		local inst = findSerializerHelperModule("FoodDefinitions")
		if not inst then
			_foodDefinitionsCache = false
			return nil
		end
	...
	local _foodServiceCache = nil
	local function getFoodService()
		if _foodServiceCache ~= nil then
			return _foodServiceCache or nil
		end

		local inst = findSerializerHelperModule("FoodService")
		if not inst then
			_foodServiceCache = false
			return nil
		end
	...
	local function ft_resolveFoodId(entry)
		if type(entry) ~= "table" then return nil end
		local candidateKeys = {
			"fid", "FoodId", "foodId", "FoodID", "foodID",
			"id", "Id", "food", "Food",
		}
		...
		local function tryFromLabel(raw)
			if type(raw) ~= "string" or raw == "" then return nil end
			local trimmed = raw:match("^%s*(.-)%s*$") or raw
			local label = trimmed
			local namePart = trimmed:match("^(.-)%s+%d+$")
			if namePart and namePart ~= "" then
				label = namePart
			end
			local defs = getFoodDefinitions()
			if defs and type(defs.Foods) == "table" then
				for foodId, def in pairs(defs.Foods) do
					if def and type(def.Label) == "string" and def.Label ~= "" then
						if def.Label == trimmed or def.Label == label then
							return foodId
						end
					end
				end
		...
		return tryFromLabel(entry.ToolName) or tryFromLabel(entry.nm) or tryFromLabel(entry.DisplayName)
	end
	...
	local function ft_candidateNames(fid)
		if not fid or fid == "" then return {} end
		local base = tostring(fid)
		return {
			base,
			base .. "Tool",
			base .. "ToolTemplate",
	...
	local function ft_findTemplate(fid)
		if not fid or fid == "" then return nil, nil end

		local function matchesTool(tool, candidates)
		if not tool or type(tool.IsA) ~= "function" or not tool:IsA("Tool") then return false end
		...
		local attr = safeGetAttribute(tool, "FoodId")
		if attr then
			for _, name in ipairs(candidates) do
				if tostring(attr) == name then
					return true
				end
			end
		end
		...
		local candidates = ft_candidateNames(fid)
		local tryFolders = {}
		...
		local function inspectFolder(folder)
			if not folder or type(folder.GetChildren) ~= "function" then return nil, nil end
			for _, candidateName in ipairs(candidates) do
				local direct = folder:FindFirstChild(candidateName)
				if direct and type(direct.IsA) == "function" and (direct:IsA("Tool") or direct:IsA("Model") or direct:IsA("Folder")) then
					return direct, folder
				end
			end

			for _, child in ipairs(folder:GetChildren()) do
				if type(child.IsA) == "function" then
					if child:IsA("Tool") and matchesTool(child, candidates) then
						return child, folder
					end
					if child:IsA("Folder") or child:IsA("Model") then
						if matchesTool(child, candidates) or tostring(child.Name) == tostring(fid) then
							return child, folder
						end
						for _, sub in ipairs(child:GetChildren()) do
							if type(sub.IsA) == "function" and sub:IsA("Tool") and matchesTool(sub, candidates) then
								return sub, child
							end
						end
					end
		...
		for _, getter in ipairs(tryFolders) do
			...
			if spec then
				return spec, container or folder
			end
		end
	...
	-- Updated to mirror FoodService template resolution and avoid fallback handles when definitions supply visuals
	local function ft_buildTool(entry, player)
		local foodId = ft_resolveFoodId(entry)
		local fallbackName = (entry and (entry.nm or entry.fid or entry.FoodId)) or foodId or "Food"

		local defs = getFoodDefinitions()
		local def = nil
		if defs then
			if type(defs.resolve) == "function" and foodId then
				local ok, resolved = pcall(defs.resolve, foodId)
				if ok then def = resolved end
			end
			if not def and defs.Foods and foodId and defs.Foods[foodId] then
				def = defs.Foods[foodId]
			end
		end

		local template, templateContainer = nil, nil
		if foodId then
			local okTemplate, spec, cont = pcall(function()
				return ft_findTemplate(foodId)
			end)
			if okTemplate then
				template, templateContainer = spec, cont
			end
		end

		if (not template) and entry and entry.ToolName then
			local okFallback, specFallback, contFallback = pcall(function()
				return ft_findTemplate(entry.ToolName)
			end)
			if okFallback then
				template, templateContainer = specFallback, contFallback
			end
		end

		local visualTemplate, visualContainer = nil, nil
		if def and def.VisualModel then
			local okVis, specVis, contVis = pcall(function()
				return ft_findTemplate(def.VisualModel)
			end)
			if okVis then
				visualTemplate, visualContainer = specVis, contVis
			end
		end

		local tool = nil
		local clonedTemplate = false
		if template and type(template.IsA) == "function" and template:IsA("Tool") then
			local okClone, clone = pcall(function() return template:Clone() end)
			if okClone and clone then
				tool = clone
				templateContainer = templateContainer or (template.Parent)
				clonedTemplate = true
				ft_dprint("ft_buildTool: cloned Tool template:", tostring(template.Name))
			end
		end

		local sourceForVisuals = template or visualTemplate

		if not tool then
			tool = Instance.new("Tool")
			tool.Name = fallbackName
			ft_dprint("ft_buildTool: instanced empty Tool for", tostring(foodId or fallbackName))
		end

		if not clonedTemplate and sourceForVisuals then
			pcall(function()
				local cloned = ft_cloneVisualsFromTemplateIntoTool(sourceForVisuals, tool)
				if cloned then
					ft_dprint("ft_buildTool: cloned visuals from template", tostring(sourceForVisuals.Name))
				end
			end)
		end

		-- Prefer template container for scripts, fall back to visual container if needed
		templateContainer = templateContainer or visualContainer

		-- Ensure handle and weld visuals (avoid spawning fallback when visuals already cloned)
		ft_ensureHandle(tool, sourceForVisuals or templateContainer, (FTCONFIG and FTCONFIG.FallbackHandleSize) or Vector3.new(1,1,1))

		-- canonical attributes (do not overwrite sensible existing values)
		safeSetAttribute(tool, "FoodItem", true)
		safeSetAttribute(tool, "FoodId", foodId or safeGetAttribute(tool, "FoodId") or fallbackName)
		...
		-- copy attributes from entry if present; otherwise fall back to definition defaults where sensible
		pcall(function()
			local currentCharges = safeGetAttribute(tool, "Charges")
			if entry and entry.ch ~= nil then
				safeSetAttribute(tool, "Charges", tonumber(entry.ch) or entry.Charges or currentCharges or (def and def.Charges) or (FTCONFIG and FTCONFIG.Charges))
			elseif entry and entry.Charges ~= nil then
				safeSetAttribute(tool, "Charges", tonumber(entry.Charges) or currentCharges or (def and def.Charges) or (FTCONFIG and FTCONFIG.Charges))
			elseif def and def.Charges ~= nil and currentCharges == nil then
				safeSetAttribute(tool, "Charges", def.Charges)
			elseif currentCharges == nil and FTCONFIG and FTCONFIG.Charges ~= nil then
				safeSetAttribute(tool, "Charges", FTCONFIG.Charges)
			end
		end)
		pcall(function()
			if entry and entry.cs ~= nil then
				safeSetAttribute(tool, "Consumable", entry.cs)
			elseif def and def.Consumable ~= nil and safeGetAttribute(tool, "Consumable") == nil then
				safeSetAttribute(tool, "Consumable", def.Consumable)
			end
		end)
		pcall(function()
			if entry and entry.rf ~= nil then
				safeSetAttribute(tool, "RestoreFraction", entry.rf)
			elseif def and def.RestoreFraction ~= nil and safeGetAttribute(tool, "RestoreFraction") == nil then
				safeSetAttribute(tool, "RestoreFraction", def.RestoreFraction)
			end
		end)
		pcall(function()
			if entry and entry.fb ~= nil then
				safeSetAttribute(tool, "FeedBufferBonus", entry.fb)
			elseif def and def.FeedBufferBonus ~= nil and safeGetAttribute(tool, "FeedBufferBonus") == nil then
				safeSetAttribute(tool, "FeedBufferBonus", def.FeedBufferBonus)
			end
		end)
		pcall(function()
			if entry and entry.cd ~= nil then
				safeSetAttribute(tool, "FeedCooldownOverride", entry.cd)
			elseif def and def.CooldownOverride ~= nil and safeGetAttribute(tool, "FeedCooldownOverride") == nil then
				safeSetAttribute(tool, "FeedCooldownOverride", def.CooldownOverride)
			end
		end)

		local desiredName = fallbackName
		if desiredName and desiredName ~= "" then
			pcall(function() tool.Name = desiredName end)
		end

		pcall(function()
			local svc = getFoodService()
			if svc and type(svc.ApplyCanonicalToolName) == "function" then
				svc.ApplyCanonicalToolName(tool, {
					foodId = foodId,
					charges = entry and (entry.ch or entry.Charges) or safeGetAttribute(tool, "Charges"),
					definition = def,
				})
			end
		end)
		...
		pcall(function()
			ft_attachLocalScriptFromContainer(templateContainer, tool)
		end)
		...
		for _, e in ipairs(list) do
			...
			if existing then
				...
				if tool then
					...
					ft_dprint("[FoodSer][RestoreImmediate] Failed to build tool for entry uid=" .. tostring(e.uid))
				end
			end
		end
	...
	local function et_buildTool(entry, player)
		...
		if tool and entry and entry.ToolName and entry.ToolName ~= "" then
			pcall(function() tool.Name = entry.ToolName end)
		end
	...
	local CAPTURE_BASE_WEIGHT_LBS = 15
	local GENERIC_CAPTURED_TOOL_NAME = "CapturedSlime"
	...
	local function computeScaleFromWeightAttribute(inst)
		...
		if weight and weight > 0 then
			local ratio = weight / CAPTURE_BASE_WEIGHT_LBS
			if ratio > 0 then
	...
	local function alignToolScaleFromWeight(inst)
		...
		if not currentMax or currentMax <= 0 or math.abs(currentMax - target) > 1e-4 then
			pcall(function() inst:SetAttribute("MaxSizeScale", target) end)
			currentMax = target
		else
			target = currentMax
		end
		...
		pcall(function() inst:SetAttribute("StartSizeScale", newStart) end)
	...
	local function formatCapturedToolName(weightLbs, slimeType, fallback)
		local weightStr = nil
		...
		local baseName = fallback or GENERIC_CAPTURED_TOOL_NAME
		if weightStr and typeStr then
			return string.format("%slb %s", weightStr, typeStr)
		elseif typeStr then
			return typeStr
		elseif weightStr then
			return string.format("%slb %s", weightStr, "Slime")
		end
		return baseName
	end

	local function cs_refreshToolAppearance(tool, entry)
		if not tool then return end
		alignToolScaleFromWeight(tool)

		local weight = nil
		if entry and tonumber(entry.WeightPounds) then
			weight = tonumber(entry.WeightPounds)
		else
			local okW, attrWeight = pcall(function() return tool:GetAttribute("WeightPounds") end)
			if okW and attrWeight then weight = tonumber(attrWeight) end
		end

		local slimeType = nil
		if entry and entry.SlimeType and entry.SlimeType ~= "" then
			slimeType = entry.SlimeType
		else
			local okT, attrType = pcall(function() return tool:GetAttribute("SlimeType") end)
			if okT and attrType and attrType ~= "" then slimeType = attrType end
		end

		local fallbackName = nil
		if entry and entry.ToolName and entry.ToolName ~= "" then
			fallbackName = entry.ToolName
		else
			fallbackName = tool.Name
		end
		tool.Name = formatCapturedToolName(weight, slimeType, fallbackName)
		pcall(function() tool:SetAttribute("DisplayName", tool.Name) end)

		local existingVisual = tool:FindFirstChild("SlimeVisual")
		if existingVisual and existingVisual:IsA("Model") then
			local desiredScale = tool:GetAttribute("CurrentSizeScale") or tool:GetAttribute("StartSizeScale")
			if not desiredScale then
				local gp = tool:GetAttribute("GrowthProgress")
				local mx = tool:GetAttribute("MaxSizeScale")
				if gp and mx then desiredScale = gp * mx end
			end
			if desiredScale then
				pcall(function() applyScaleToModel(existingVisual, desiredScale) end)
			end
		end
	end

	local function cs_applyEntryAttributes(tool, entry, player)
		if not tool then return end
		...
		local function copyAttr(name, value, asNumber)
			...
			pcall(function() tool:SetAttribute(name, finalValue) end)
		end

		copyAttr("SlimeId", entry.SlimeId, false)
		copyAttr("ToolUniqueId", entry.ToolUniqueId or entry.ToolUid, false)
		copyAttr("CapturedAt", entry.CapturedAt, true)
		copyAttr("Rarity", entry.Rarity, false)
		copyAttr("SlimeType", entry.SlimeType, false)
		copyAttr("WeightPounds", entry.WeightPounds, true)
		copyAttr("CurrentValue", entry.CurrentValue, true)
		copyAttr("GrowthProgress", entry.GrowthProgress, true)
		copyAttr("ValueFull", entry.ValueFull, true)
		copyAttr("ValueBase", entry.ValueBase, true)
		copyAttr("ValuePerGrowth", entry.ValuePerGrowth, true)
		copyAttr("MaxSizeScale", entry.MaxSizeScale, true)
		copyAttr("StartSizeScale", entry.StartSizeScale, true)
		copyAttr("CurrentSizeScale", entry.CurrentSizeScale, true)
		copyAttr("BodyColor", entry.BodyColor, false)
		copyAttr("AccentColor", entry.AccentColor, false)
		copyAttr("EyeColor", entry.EyeColor, false)
		copyAttr("DisplayName", entry.DisplayName or entry.ToolName, false)
	end
	...
	local function cs_buildTool(entry, player)
		...
		cs_applyEntryAttributes(tool, entry, player)

		cs_refreshToolAppearance(tool, entry)
	...
	local function cs_serialize(player, isFinal, profile)
		...
		for _, tool in ipairs(tools) do
			...
			if not skip then
				...
				pcall(function() entry.ToolName = tool.Name end)
				pcall(function() entry.DisplayName = tool:GetAttribute("DisplayName") end)
				pcall(function() entry.Rarity = tool:GetAttribute("Rarity") end)
				pcall(function() entry.SlimeType = tool:GetAttribute("SlimeType") end)
				...
				pcall(function() entry.ValueFull = tool:GetAttribute("ValueFull") end)
				pcall(function() entry.ValueBase = tool:GetAttribute("ValueBase") end)
				pcall(function() entry.ValuePerGrowth = tool:GetAttribute("ValuePerGrowth") end)
				pcall(function() entry.MaxSizeScale = tool:GetAttribute("MaxSizeScale") end)
				pcall(function() entry.StartSizeScale = tool:GetAttribute("StartSizeScale") end)
				pcall(function() entry.CurrentSizeScale = tool:GetAttribute("CurrentSizeScale") end)
			...
			if not entry then
				...
				if existing then
					pcall(function()
						...
						cs_applyEntryAttributes(existing, entry, player)
						cs_refreshToolAppearance(existing, entry)
						pcall(function() existing.Parent = backpack end)
	...
	local _preExitHelpersCache = nil
	local function getPreExitHelpers()
		if _preExitHelpersCache ~= nil then
			return _preExitHelpersCache
		end

		local inst = findSerializerHelperModule("GrandInventorySerializerPreExitHelpers")
		if not inst then
			_preExitHelpersCache = false
			return nil
		end
		...
		if not (ok and type(builder) == "function") then
			_preExitHelpersCache = false
			return nil
		end

		local okBuild, helpers = pcall(builder, {
			sanitizeInventoryOnProfile = sanitizeInventoryOnProfile,
			getPPS = getPPS,
			we_enumeratePlotEggs = we_enumeratePlotEggs,
			we_enumeratePlotEggs_by_userid = we_enumeratePlotEggs_by_userid,
			safe_get_profile = safe_get_profile,
			dprint = dprint,
		})
		if okBuild and type(helpers) == "table" then
			_preExitHelpersCache = helpers
		else
			_preExitHelpersCache = false
		end
	...
	local function preExitSyncFallback(a1, a2)
	...
	function GrandInventorySerializer.PreExitSync(...)
		local a1, a2 = _normalize_call_args({ ... })
		local helpers = getPreExitHelpers()
		if helpers and helpers.preExitSync then
			local ok, result = pcall(function()
				return helpers.preExitSync(a1, a2)
			end)
			if ok then
				return result
			else
				dprint("PreExitSync helper execution failed; falling back")
			end
		elseif helpers == false then
			dprint("PreExitSync helper unavailable; using fallback implementation")
		end

		return preExitSyncFallback(a1, a2)
	end


	local _eggWatcherCache = nil
	local function ensureEggWatcher()
		if _eggWatcherCache ~= nil then
			return _eggWatcherCache
		end

		local inst = findSerializerHelperModule("GrandInventorySerializerEggWatcher")
		if not inst then
			_eggWatcherCache = false
			return nil
		end

		local ok, initializer = pcall(function()
			return require(inst)
		end)
		if not (ok and type(initializer) == "function") then
			_eggWatcherCache = false
			return nil
		end

		local okInit, watcher = pcall(initializer, {
			Workspace = Workspace,
			ServerScriptService = ServerScriptService,
			getPPS = getPPS,
		})
		if okInit and watcher then
			_eggWatcherCache = watcher
		else
			_eggWatcherCache = false
		end
	...
	do
		local watcher = ensureEggWatcher()
		if not watcher then
			dprint("Egg watcher helper unavailable; egg removal sync disabled")
		end
	end
