...
-- Features:
--   * Standing-based payout multipliers (persisted via PlayerDataService)
--   * Optional color preference multiplier
--   * Uses CurrentValue if present (else derives from ValueBase + ValuePerGrowth)
--   * Dual remote response (table payload + legacy positional params)
--   * Request token echo
--   * Faction registration with PlayerDataService
--
-- Updated:
--   * Fixed "Unknown global 'player'" warning (buildPayload now receives requesting player explicitly)
--   * Safer failure path; includes standing snapshot
--   * Optional immediate persistence after sale (SaveImmediately) if available
--   * Idempotent remote ensuring (optional – remove if handled elsewhere)
...
----------------------------------------------------------------
local PlayerDataService = require(ServerScriptService:WaitForChild("Modules"):WaitForChild("PlayerDataService"))
...
-- Register factions with PlayerDataService
for name, cfg in pairs(FACTIONS) do
	PlayerDataService.RegisterFaction(name, cfg.StandingInitial or 0)
end
...
local function processSale(player, faction, toolList)
	...
	if FactionTotalsService and FactionTotalsService.AddPayout then
		pcall(function()
			FactionTotalsService.AddPayout(faction, totalPayout)
		end)
	end

	-- Award coins via leaderstats (PlayerDataService leaderstats bridge will persist)
	local ls = player:FindFirstChild("leaderstats")
	if ls then
		local coins = ls:FindFirstChild("Coins")
		if coins and coins:IsA("IntValue") then
			coins.Value += totalPayout
		end
	end
	...
	-- Optional immediate persistence (coins/standing already bridged; this forces earlier save)
	if PlayerDataService.SaveImmediately then
		PlayerDataService.SaveImmediately(player, "FactionSale")
	end
...
local function buildPayload(requestingPlayer, success, factionName, dataOrMessage, requestToken)
	if success then
		-- dataOrMessage is the sale result table
		return {
			success        = true,
			faction        = factionName,
