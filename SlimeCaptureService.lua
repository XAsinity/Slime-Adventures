...
-- Version: v2.4 (adds PersistentCaptured + SlimmedVisualParts attr + instrumentation)
--
-- Includes: Rarity + full attribute + hex color normalization
-- Adds: PersistentCaptured=true on tool so other cleanup scripts can skip these.
--
...
local PlayerDataService  = require(ServerModules:WaitForChild("PlayerDataService"))
...
local function perform(player, slime)
	...
	PlayerDataService.MarkDirty(player, "CapturedSlime")
	...
	if CONFIG.ImmediateSave then
		...
		PlayerDataService.SaveImmediately(player, CONFIG.SaveReason)
	end
...
function SlimeCaptureService.TestCapture(player, slime)
	return perform(player, slime)
end
